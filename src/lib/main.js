window.onload = function () {
  ...
  function renderStats() {
    ...
    var todayData = stats[today] || { focus: 0, focusMin: 0, break: 0, breakMin: 0 };
    ...
    // Weekly stats
    var days = [];
    var weeklyFocusTotal = 0;
    var weeklyBreakTotal = 0;
    for (var i = 6; i >= 0; i--) {
      var d = new Date();
      d.setDate(d.getDate() - i);
      var key = d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0');
      var dayData = stats[key] || { focus: 0, break: 0 };
    ...
    // Draw line chart
    var chart = document.querySelector('#weekly-chart');
    ...
    for (var x = 0; x < days.length; x++) {
      var xPos = padL + (w / (days.length - 1)) * x;
      var shortDate = days[x].date.split(' ')[0];
    ...
    for (var p = 0; p < days.length; p++) {
      var px = padL + (w / (days.length - 1)) * p;
      var py = padT + h - (h * days[p].focus / maxVal);
    ...
    for (var d2 = 0; d2 < days.length; d2++) {
      var dx = padL + (w / (days.length - 1)) * d2;
      var dy = padT + h - (h * days[d2].focus / maxVal);
      cctx.beginPath();
    ...
    var totalHtml = '<span class="weekly-stats-date">합계</span>';
